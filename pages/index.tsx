...
export default function HomePage() {
  ...
  return (
    <div className="max-w-xl mx-auto py-8 px-2">
      ...
      <form onSubmit={createTrip} className="bg-white/70 rounded-2xl shadow-lg p-4 flex flex-col gap-3 mb-8">
        <label className="font-bold text-xl">Název výletu</label>
        <input className="rounded-xl p-2 border" value={name} onChange={e => setName(e.target.value)} placeholder="např. Sicílie 2025" />
        ...
        {members.map((m, i) => (
          <input
            ...
            value={m.name}
            onChange={e => {
              setMembers(prev => prev.map((mm, j) => j === i ? { ...mm, name: e.target.value } : mm));
            }}
        ...
        <button type="submit" className="bg-green-500 text-white px-4 py-2 rounded-xl font-bold hover:bg-green-600 transition mt-3">
          Vytvořit výlet
        </button>
      </form>
      <div>
        ...
        <ul className="flex flex-col gap-2">
          {trips.map(trip => (
            <li key={trip.id} className="bg-white/60 rounded-xl p-2 shadow flex justify-between items-center">
              <span>{trip.name}</span>
              <a href={`/trip/${trip.id}`} className="text-purple-700 font-bold hover:underline">Zobrazit</a>
            </li>
          ))}
        </ul>
      </div>
